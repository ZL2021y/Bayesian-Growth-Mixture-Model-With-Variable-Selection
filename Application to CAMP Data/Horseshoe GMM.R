...
bayes.horseshoe <- function(	mydat,				# trajectory data  
...
# initialization
tau2 <- rep(1,num.cluster-1)
lambda2 <- matrix(1,ncol=p,nrow=num.cluster-1)
v <-  matrix(1,ncol=p,nrow=num.cluster-1)
xi <-  rep(1,p)
...
cat('Running Horseshoe Joint Modeling Method', '\n')
cat(paste(rep('-',60),sep='',collapse=''), '\n'); cat(paste(rep('-',60),sep='',collapse=''), '\n'); 
...
# Sample beta (WITHOUT THE INTERCEPT)
	Omega <- diag(w[,j]) 
	eta <- kappa[,j]/w[,j]
	D.star <- tau2[j]*diag(lambda2[j,])
	V <- solve(t(covar) %*% Omega %*% covar + solve(D.star,tol=1e-20))
...
#--------------------------------------------------------------#
for (j in 1:(num.cluster-1)) {
	for (s in 1:p){lambda2[j,s] <- rinvgamma(n=1, shape=1, scale = 1/v[j,s] + beta[j,s]^2/(2*tau2[j]))}}
#--------------------------------------------------------------#
# sample tau2
#--------------------------------------------------------------#		
for (j in 1:(num.cluster-1)){tau2[j] <- rinvgamma(n=1, shape=(p+1)/2, scale = 1/xi[j] + (1/2)*sum(beta[j,]^2/lambda2[j,]))}
#--------------------------------------------------------------#
# sample v
#--------------------------------------------------------------#
for (j in 1:(num.cluster-1)){	
	for (s in 1:p){v[j,s] <- rinvgamma(n=1, shape=1, scale = 1+1/lambda2[j,s])}}
#--------------------------------------------------------------#
# sample xi
#--------------------------------------------------------------#	
for (j in 1:(num.cluster-1)){	xi[j] <- rinvgamma(n=1, shape=1, scale = 1+1/tau2[j])}
